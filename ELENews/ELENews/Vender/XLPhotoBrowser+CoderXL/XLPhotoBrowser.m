...
- (UIButton *)backBtn{
    if (!_backBtn) {
        ...
        [_backBtn setImage:[UIImage imageNamed:@"black_circle_back_33x33_"] forState:UIControlStateNormal];
...
- (void)iniaialUI
{
    XLLogFunc;
    if (_rootView) {
        [_rootView addSubview:self.backBtn];
    } else {
        [self.photoBrowserWindow addSubview:self.backBtn];
    }
    
    self.backBtn.frame = CGRectMake(15, 25, 33, 33);
...
- (void)backBtnClicked{
    if (_delegate && [_delegate respondsToSelector:@selector(photoBrowserClickBackBtn:)]) {
        [_delegate photoBrowserClickBackBtn:self];
    }
}
...
{
    // 实现代理时不做处理，交给外部处理
    if (_delegate && [_delegate respondsToSelector:@selector(photoBrowserClickSinglePhoto:)]) {
        [_delegate photoBrowserClickSinglePhoto:self];
        return;
    }
    // 没有实现代理，关闭浏览器
 ...
 *
 */
+ (instancetype)showPhotoBrowserWithCurrentImageIndex:(NSInteger)currentImageIndex imageCount:(NSUInteger)imageCount datasource:(id<XLPhotoBrowserDatasource>)datasource rootView:(UIView *)rootView
{
    XLPhotoBrowser *browser = nil;
    if (rootView) {
        browser = [[XLPhotoBrowser alloc] initWithRootView:rootView];
    } else {
        browser = [[XLPhotoBrowser alloc] init];
    }
 ...
 */
+ (instancetype)showPhotoBrowserWithImages:(NSArray *)images currentImageIndex:(NSInteger)currentImageIndex rootView:(UIView *)rootView
{
    ...
    XLPhotoBrowser *browser = nil;
    if (rootView) {
        browser = [[self alloc] initWithRootView:rootView];
    } else {
