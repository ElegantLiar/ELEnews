...
- (void)setShowHighQualityImageWithURL:(NSURL *)url placeholderImage:(UIImage *)placeholder
{
    ...
    UIImage *cacheImage = [[YYImageCache sharedCache] getImageForKey:[url absoluteString]];
    if (cacheImage) {
    ...
    [weakSelf.photoImageView yy_setImageWithURL:url
                                    placeholder:placeholder
                                        options:YYWebImageOptionHandleCookies|YYWebImageOptionIgnoreFailedURL manager:[YYWebImageManager sharedManager] progress:^(NSInteger receivedSize, NSInteger expectedSize) {
                                     ...
                                     completion:^(UIImage * _Nullable image, NSURL * _Nonnull url, YYWebImageFromType from, YYWebImageStage stage, NSError * _Nullable error) {
                                         ...
                                         if (error) {
                                             ...
                                             XLPBLog(@"加载图片失败 , 图片链接imageURL = %@ , 错误信息: %@ ,检查是否开启允许HTTP请求",url,error);
                                         } else {
